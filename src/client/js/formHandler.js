...
async function handleSubmit(event) {
  ...
  if (checkForUrl(formUrl)) {
    ...
    fetch('http://localhost:8081/article', {
      ...
      alert (
        'The URL you have entered is invalid. Please enter a valid URL.'
      );
    }
    ...
    async function updateUI (res) {
      ...
      document.querySelector('#polarity').innerText = `Polarity: ${PolarityCheckerResults(
        res.score_tag
      )}`
    }
